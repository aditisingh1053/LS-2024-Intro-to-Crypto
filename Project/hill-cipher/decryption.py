...
def gaussian_elimination(matrix_a, matrix_b):
    ...
    augmented_matrix = np.hstack((matrix_a, matrix_b)).astype(float)  # Combine A and B into an augmented matrix
...
def find_key_matrix(plain_mat, cipher_mat):
...
def main():
    ...
    plain_matrix = prepare_text_matrix(plaintext)
    cipher_matrix = prepare_text_matrix(ciphertext)
